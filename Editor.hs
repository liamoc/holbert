...
data Editor = Editor { document :: Document, currentFocus :: EditorFocus, inputText :: InputText, message :: Maybe ErrorMessage, displayOptions :: DisplayOptions }
            deriving (Show, Eq)

data EditorFocus = ItemFocus ItemIndex (Focus I.Item)
                 | NoFocus
...
data EditorAction = SetFocus EditorFocus
...
rulesSummary (i,p) s = 
 ...
 where
   ...
   groupedRules (i:xs) (h:acc) = let rs = map (\(R.R n prp _) -> (PT.Defn n, prp)) (toListOf I.rule i)
...
switchFocus (ItemFocus idx f) ed =  ed { currentFocus = ItemFocus idx f , inputText = fromMaybe "" (editable f (document ed !! idx)) }
...
runAction' (InsertItem idx itm) ed = let 
           (first,last) = splitAt (idx + 1) (document ed) 
        in pure $ switchFocus (ItemFocus (idx + 1) $ inserted itm) (ed { document = first ++ itm:last, message = Nothing})
runAction' (ShiftDown idx) ed = let 
           (lefts,x:y:rest) = splitAt idx (document ed)
           y' = foldr (.) id (map invalidated (defined x)) y
        in pure (ed { document = lefts ++ y':x:rest, currentFocus = NoFocus, message = Nothing } )
runAction' (DeleteItem idx) ed =  let
           (lefts,x:rest) = splitAt idx (document ed)
           rest' = map (foldr (.) id (map invalidated (defined x))) rest
        in pure (ed { document = lefts ++ rest', currentFocus = NoFocus, message = Nothing } )
runAction' (InsertProposition idx b) ed = do
